...
import type { Arcana } from "@/lib/mock-data";

export const Route = createFileRoute("/library")({
  head: () => ({
    meta: [
      { title: "Card Library — Arcana Journey" },
      { name: "description", content: "Browse the full deck of tarot cards, from the Major Arcana to the four suits." },
      { property: "og:title", content: "Card Library — Arcana Journey" },
      { property: "og:description", content: "Browse the full deck of tarot cards." },
    ],
  }),
    ...
    <div className="mx-auto max-w-6xl px-6 py-16">
      ...
      {isLoading ? <p className="text-center text-brown-soft">Shuffling the deck...</p> : (
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-8">
          {filtered.map((c) => (
            <Link key={c.id} to="/library/$id" params={{ id: String(c.id) }} className="group flex flex-col items-center gap-3 transition-transform hover:-translate-y-1">
