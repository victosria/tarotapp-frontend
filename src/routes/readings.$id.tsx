...
export const Route = createFileRoute("/readings/$id")({
  head: () => ({
    meta: [
      { title: "Reading — Arcana Journey" },
      { name: "description", content: "A saved tarot reading from your journal." },
      { property: "og:title", content: "Reading — Arcana Journey" },
      { property: "og:description", content: "A saved tarot reading from your journal." },
    ],
  }),
...
function ReadingDetailPage() {
  const { id } = useParams({ from: "/readings/$id" });
  ...
  const del = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["readings"] }); nav({ to: "/readings" }); },
  });
  ...
  return (
    <div className="mx-auto max-w-5xl px-6 py-16">
      <Link to="/readings" className="text-xs uppercase tracking-[0.25em] text-brown-soft hover:text-gold-deep">← Back to Journal</Link>
