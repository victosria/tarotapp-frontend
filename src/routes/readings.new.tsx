...
const searchSchema = z.object({ spreadId: z.coerce.number().optional() });

export const Route = createFileRoute("/readings/new")({
  validateSearch: searchSchema,
  head: () => ({
    meta: [
      { title: "New Reading — Arcana Journey" },
      { name: "description", content: "Cast a new tarot reading and record it in your journal." },
      { property: "og:title", content: "New Reading — Arcana Journey" },
      { property: "og:description", content: "Cast a new tarot reading." },
    ],
  }),
...
function NewReadingPage() {
  const { spreadId } = useSearch({ from: "/readings/new" });
  ...
  const [selectedSpreadId, setSelectedSpreadId] = useState<number | null>(spreadId ?? null);
  ...
  const create = useMutation({
    ...
    onSuccess: (r) => {
      ...
      nav({ to: "/readings/$id", params: { id: String(r.id) } });
    },
  });
    ...
    <div className="mx-auto max-w-5xl px-6 py-16">
      ...
      <form className="space-y-8 mt-12" onSubmit={(e) => { e.preventDefault(); create.mutate(); }}>
        ...
        <div className="flex justify-end gap-3">
          <button type="button" onClick={() => nav({ to: "/readings" })} className="rounded-full border border-brown/30 px-6 py-3 text-xs uppercase tracking-widest text-brown hover:bg-cream-deep">Cancel</button>
