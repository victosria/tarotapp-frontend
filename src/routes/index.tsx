...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      { title: "Arcana Journey — A Tarot Journal" },
      { name: "description", content: "Explore the arcana, save your readings, and keep a mystical journal of the cards that speak to you." },
      { property: "og:title", content: "Arcana Journey — A Tarot Journal" },
      { property: "og:description", content: "Explore the arcana, save your readings, and keep a mystical journal." },
    ],
  }),
  component: Index,
});
          ...
          <div className="mx-auto max-w-6xl px-6 pt-20 pb-24 grid md:grid-cols-2 gap-10 items-center relative">
            <div className="space-y-6">
              ...
              <div className="flex gap-4 pt-2">
                <Link to="/readings/new" className="rounded-full bg-gradient-gold text-cream px-6 py-3 text-sm uppercase tracking-widest shadow-[0_15px_30px_-15px_rgba(120,80,20,0.5)]">Draw a card</Link>
                <Link to="/library" className="rounded-full border border-brown/30 text-brown px-6 py-3 text-sm uppercase tracking-widest hover:bg-cream-deep">Browse library</Link>
              </div>
            </div>
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {FEATURES.map((f) => (
              <div key={f.title} className="rounded-2xl bg-card border border-[color-mix(in_oklab,var(--gold)_25%,transparent)] p-8 space-y-3">
                ...
                <Link to={f.href} className="inline-block text-xs uppercase tracking-[0.25em] text-gold-deep pt-2">{f.cta} →</Link>
              </div>
            ))}
          </div>
        </section>
