...
export const Route = createFileRoute("/favorites")({
  head: () => ({
    meta: [
      { title: "Favorites — Arcana Journey" },
      { name: "description", content: "The tarot cards that speak most closely to your soul, kept in one place." },
      { property: "og:title", content: "Favorites — Arcana Journey" },
      { property: "og:description", content: "Your favorite tarot cards." },
    ],
  }),
...
function FavoritesPage() {
  ...
  const remove = useMutation({
    mutationFn: (id: number) => apiRemoveFavorite(id),
    ...
    <div className="mx-auto max-w-6xl px-6 py-16">
      ...
      {isLoading ? <p className="text-center text-brown-soft mt-10">Gathering...</p> : favs.length === 0 ? (
        <div className="text-center py-16 rounded-2xl border border-dashed border-[color-mix(in_oklab,var(--gold)_35%,transparent)] mt-10">
          ...
          <p className="text-brown-soft mt-2">Browse the <Link to="/library" className="text-gold-deep underline decoration-dotted">library</Link> and save the cards that call to you.</p>
        </div>
      ) : (
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-8 mt-12">
          {favs.map((f) => (
            <div key={f.id} className="flex flex-col items-center gap-3">
              <Link to="/library/$id" params={{ id: String(f.card.id) }}>
