      ...
      <div className="grid md:grid-cols-2 gap-6 mt-12">
        {spreads.map((s) => (
          <div key={s.id} className="rounded-2xl bg-card border border-[color-mix(in_oklab,var(--gold)_25%,transparent)] p-8 space-y-4">
            ...
            <Link href={{ pathname: '/readings/new', query: { spreadId: s.id } }} className="inline-block mt-2 rounded-full bg-gradient-gold text-cream px-5 py-2 text-xs uppercase tracking-widest">Cast this spread</Link>
          </div>
        ))}
      </div>
    </div>
